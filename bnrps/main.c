...
bnrps_logfunc_t logmsg = stderr_logfunc;
...
static int scanfunc(void *usr_data, const uint8_t mac[6],
  ...
  if ((phy_conf & BNRPS_PHY_PROBE_10MBIT) && (phy_conf != BNRPS_PHY_PROBE_AUTO)) {
    printf("PROBE_10MBIT");
  }
  if ((phy_conf & BNRPS_PHY_PROBE_100MBIT) && (phy_conf != BNRPS_PHY_PROBE_AUTO)) {
    if (phy_conf & BNRPS_PHY_PROBE_10MBIT)
      printf(" | ");
    printf("PROBE_100MBIT");
  }
  else if (phy_conf == BNRPS_PHY_PROBE_AUTO) {
    printf("PROBE_AUTO");
  }
  printf(")\n");

  printf(" phy state: 0x%02x (", phy_state);
  if (phy_state & BNRPS_PHY_LINK_BEAT)
    printf("LINK_BEAT | ");
  if (phy_state & BNRPS_PHY_LINK_100MBIT)
    printf("LINK_100MBIT");
  else
    printf("LINK_10MBIT");
  if (phy_state & BNRPS_PHY_LINK_FDX)
    printf(" | LINK_FDX");
  printf(")\n");
  ...
  if (options & BNRPS_OPT_DHCP) {
    printf("(OPT_DHCP");
    opt = 1;
  }

  if (options & BNRPS_OPT_HTTP_AUTH) {
    if (opt)
      printf(" | ");
    else
      printf("(");
...
static struct {
  ...
  {'m', "mac address of the device (notation: XX:XX:XX:XX:XX:XX)", NULL, "mac", AC_DEFAULT, {0, 3, 3, 3, 3, 0, 0, 0}},
  {'h', "hostname or ip address of the device (default: 255.255.255.255)", NULL, "host", AC_DEFAULT, {2, 4, 4, 4, 3, 3, 3, 3}},
  {'i', "ip address (default: 0.0.0.0)", NULL, "ip", AC_NETCONF, {0, 2, 0, 0, 0, 0, 0, 0}},
  {'n', "netmask address", NULL, "netmask", AC_NETCONF, {0, 2, 0, 0, 0, 0, 0, 0}},
  {'g', "gateway address (default: 0.0.0.0)", NULL, "gateway", AC_NETCONF, {0, 2, 0, 0, 0, 0, 0, 0}},
  {'e', "ethernet phy configuration (values: 0=auto; 1=10mbit; 2=100mbit; default: 0)", NULL, "phy", AC_NETCONF, {0, 2, 0, 0, 0, 0, 0, 0}},
  {'d', "enable dhcp (1=on; 0=off; default: 1)", NULL, "status", AC_NETCONF, {0, 2, 0, 0, 0, 0, 0, 0}},
  {'a', "enable http auth (1=on; 0=off; default: 0)", NULL, "status", AC_NETCONF, {0, 2, 0, 0, 0, 0, 0, 0}},
  {'p', "http port (default: 80)", NULL, "port", AC_DEFAULT|AC_NETCONF, {0, 2, 0, 0, 0, 2, 2, 2}},
  {'u', "authentification data (notation: <username>:<password>)", NULL, "usrpwd", AC_DEFAULT, {0, 0, 0, 0, 0, 2, 2, 2}},
  {'c', "continuous operation", NULL, NULL, AC_DEFAULT, {2, 0, 2, 0, 0, 0, 2, 2}},
  {'w', "wait 'time' seconds between continuous operations (default: 2)", NULL, "time", AC_DEFAULT, {2, 0, 2, 0, 0, 0, 2, 2}},
  {'t', "timeout in seconds (default: 3)", NULL, "sec", AC_DEFAULT, {2, 2, 2, 0, 2, 2, 2, 2}},
  {'s', "enable syslog output", NULL, NULL, AC_DEFAULT, {2, 2, 2, 2, 2, 2, 2, 2}},
  {'x', "write scan results to " CONFFILE, NULL, NULL, AC_DEFAULT, {2, 0, 0, 0, 0, 0, 0, 0}},
  {'f', "load mac and/or ip address from " CONFFILE, NULL, "id", AC_DEFAULT, {0, 5, 5, 5, 5, 5, 5, 5}},
  {0, NULL, NULL, NULL, 0, {0, 0, 0, 0, 0, 0, 0, 0}},
};

/* corresponding getopts_long string */
#define SHORTOPTS "m:h:i:n:g:e:d:a:p:u:cw:t:sxf:"
...
static struct {
  ...
  {"power3", "state of powerport 3 (0=off; 1=on)"},
  {"power4", "state of powerport 4 (0=off; 1=on)"},
  {"power5", "state of powerport 5 (0=off; 1=on)"},
  {"power6", "state of powerport 6 (0=off; 1=on)"},
  {"power7", "state of powerport 7 (0=off; 1=on)"},
  {"power8", "state of powerport 8 (0=off; 1=on)"},
...
static void show_keys(const int inv) {
  ...
  for(i=0; i<BNRPS_MAX_KEYS; i++) {
	...
	break;
      default:
	fprintf(stderr, "<%s>", bnrps_confmap[i].key);
	len += 2 + strlen(bnrps_confmap[i].key);
	break;
      }
    }
    else
...
static int parse_mac (char *mac, bnrps_dev_t *dev)
{
	if (strlen(mac) != 17) {
...
static int parse_host (char *hostname, bnrps_dev_t *dev)
{
...
int main(int argc, char **argv) {
  int i = 0;
  bnrps_mode_e mode = MODE_NONE;
  char *mode_arg;

  bnrps_log_sethandler(logmsg);
  ...
  int phy_conf = BNRPS_PHY_PROBE_AUTO;
  int options = BNRPS_OPT_DHCP;
  int loop = 0;
  int detach = 0;
  int iwt = 2;
  int has_netmask = 0;
  ...
  for(i=0; shortopts[i].c; i++) {
	...
	case 0:
	  phy_conf = BNRPS_PHY_PROBE_AUTO;
	  break;
	case 1:
	  phy_conf = BNRPS_PHY_PROBE_10MBIT;
	  break;
	case 2:
	  phy_conf = BNRPS_PHY_PROBE_100MBIT;
	  break;
	case 3:
	  phy_conf = BNRPS_PHY_PROBE_10MBIT | BNRPS_PHY_PROBE_100MBIT;
	  break;
	default:
      ...
      case 't':
	{
	  ...
	  bnrps_set_timeout(NULL, h);
	  bnrps_set_timeout(&dev, h);
	}
      case 's':
	logmsg = syslog_logfunc;
	break;
  ...
  do {
	...
	if (bnrps_scan(inet_ntoa(dev.address), scanfunc, &arg) == BNRPS_RET_OK) {
	  logmsg(LOG_NOTICE, "\n%d device%s found.\n\n", arg.cnt, arg.cnt == 1 ? "" : "s");
	  ret = 0;
	}
	else {
    ...
    case MODE_NETCONF:
      if (bnrps_netconf(&dev, address, netmask, gateway, phy_conf, options, dev.port) == BNRPS_RET_OK) {
    ...
    case MODE_PING:
      if (bnrps_ping(&dev) == BNRPS_RET_OK) {
    ...
    case MODE_RESET:
      if (bnrps_reset(&dev) != BNRPS_RET_OK) {
	...
	if (fd == -1) {
	  logmsg(LOG_WARNING, "Could not open firmware fiel: %s\n", strerror(errno));
	  return 1;
	}

	int flashmode = -1;

	if (bnrps_scan(inet_ntoa(dev.address), modecheckfunc, &flashmode) != BNRPS_RET_OK || flashmode == -1) {
	...
	if (bnrps_flash(&dev, fd) != BNRPS_RET_OK) {
	  close(fd);
	...
	bnrps_set_t set;
	char *err;
	if (bnrps_key_set_prep(&set, i, val, &err) == BNRPS_RET_ERR) {
	  logmsg(LOG_WARNING, "Setting %s failed: %s\n", mode_arg, err);
	  return 1;
	}
	if (bnrps_key_set_do(&dev, &set) != BNRPS_RET_OK) {
	...
	if (mode == MODE_GET) {
	  ...
	  if ((i == BNRPS_MAX_KEYS) || (!bnrps_confmap[i].macro)) {
	    show_keys(3);
	    return 1;
	  }
	}

	if (bnrps_key_dump(&dev, &keyvals) != BNRPS_RET_OK)
